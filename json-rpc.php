...
function handle_json_rpc($object) {
  ...
  */
  ...
  if (!$input) {
    if ($GLOBALS['HTTP_RAW_POST_DATA'] == "") {
      echo response(null, 0, array("code"=> -32700,
				   "message"=>"Ошибка разбора строки: нет данных"));
    } else {
      // json parse error
      $error = json_error();
      $id = extract_id();
      echo response(null, $id, array("code"=> -32700,
				     "message"=>"Ошибка разбора строки: $error"));
    }
    exit;
  } else {
  ...
  try {
    ...
    if (strcmp($method, 'help') == 0) {
      if (count($params) > 0) {
        if (!in_array($params[0], $methods)) {
          $no_method = 'Команды ' . $params[0] . ' не найдено';
          ...
          if (array_key_exists($help_str_name, $static)) {
            ...
            throw new Exception($method . " команда не документирована");
          }
        }
      } else {
    ...
    $msg = "Внутренняя ошибка: " . $e->getMessage();
